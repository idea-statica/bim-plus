...
namespace AllplanBimplusDemo.UserControls
{
	...
	public partial class CsgObjectsControl : NotifyPropertyChangedUserControl
	{
		...
		internal IdeaStatiCaController IdeaController { get => ideaController; set => ideaController = value; }
		...
		public void LoadContent(IntegrationBase integrationBase, WPFWindows.Window parent)
		{
			...
			IdeaController = new IdeaStatiCaController(integrationBase);
		}
		...
		private void DeleteConnectionBtn_Click(object sender, RoutedEventArgs e)
		{
			...
			ProgressWindow.Text = "Deleting IdeaStatica Connection";
			ProgressWindow.Show();
			...
			if (connection != null)
			{
				DeleteConnection(model, new List<ConnectionData> { connection }, connectionId);
			}
		...
		private void DeleteConnection(DtoDivision model, List<ConnectionData> connections, int? connectionId = null)
		{
			try
			{
